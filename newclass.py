...
class DocumentProcessor:
    ...
    def analyze_document(self, file_path: str) -> str:
        """
        Analyze a document and return insights about its content.
        ...
        try:
            ...
            if file_type == "text":
                ...
                with open(file_path, 'r', encoding='utf-8', errors='ignore') as f:
                    content = f.read(10000)  # Read first 10K chars for analysis
                ...
                # Extract a preview/summary (first few lines)
                ...
                analysis = (
                    f"Document Analysis:\n\n"
                    f"File: {file_name}\n"
                    f"Type: {extension.upper()} (Text Document)\n"
                    f"Size: {file_size / 1024:.1f} KB\n"
                    f"Statistics: {line_count} lines, {word_count} words, {char_count} characters\n\n"
                    f"Content Preview:\n{preview}\n\n"
                    f"This document has been processed and indexed for retrieval in future queries."
                )
                ...
                # Attempt to extract basic info from PDF
                try:
                    loader = PyPDFLoader(file_path)
                    documents = loader.load()
                    ...
                    if documents:
                        ...
                        preview = "No text content could be extracted"
                    ...
                    analysis = (
                        ...
                        f"This document has been processed and indexed for retrieval.\n"
                        f"You can ask questions about its content in your next message."
                    )
                except Exception as pdf_err:
                    # Fallback if detailed extraction fails
                    logger.error(f"Error in PDF extraction: {str(pdf_err)}")
                    analysis = (
                        f"PDF Document Analysis:\n\n"
                        f"File: {file_name}\n"
                        f"Type: PDF Document\n"
                ...
                analysis = (
                    ...
                    f"File: {file_name}\n"
                    f"Language: {extension.upper()}\n"
                    f"Size: {file_size / 1024:.1f} KB\n"
                    f"Lines of Code: {line_count}\n\n"
                    f"Code Preview:\n{preview}\n\n"
                    f"This code file has been processed and indexed.\n"
                    f"You can ask questions about the code in your next message."
                )
                ...
                if extension.lower() == "csv":
                    try:
                        ...
                        if headers:
                            ...
                            analysis = (
                                ...
                                f"This data file has been processed and indexed.\n"
                                f"You can ask questions about this data in your next message."
                            )
                    except Exception as csv_err:
                        logger.error(f"Error in CSV analysis: {str(csv_err)}")
                        analysis = (
                    ...
                    try:
                        ...
                        # Create basic structure overview
                        if isinstance(json_data, dict):
                            keys = ", ".join(list(json_data.keys())[:10])
                            structure = f"Object with keys: {keys}"
                            if len(json_data) > 10:
                                structure += f" and {len(json_data) - 10} more"
                        elif isinstance(json_data, list):
                            structure = f"Array with {len(json_data)} items"
                            if json_data and isinstance(json_data[0], dict):
                                sample_keys = ", ".join(list(json_data[0].keys())[:5])
                                structure += f" (Sample item keys: {sample_keys}...)"
                        else:
                            structure = "Simple value"
                            
                        analysis = (
                            f"JSON Data Analysis:\n\n"
                            f"File: {file_name}\n"
                            f"Size: {file_size / 1024:.1f} KB\n"
                            f"Structure: {structure}\n\n"
                            f"This JSON file has been processed and indexed.\n"
                            f"You can ask questions about this data in your next message."
                        )
                    except Exception as json_err:
                        logger.error(f"Error in JSON analysis: {str(json_err)}")
                        analysis = (
                            f"Data File Analysis:\n\n"
                            f"File: {file_name}\n"
                            f"Type: JSON\n"
                ...
                analysis = (
                    f"Image Analysis:\n\n"
                    f"File: {file_name}\n"
                    f"Type: {extension.upper()} Image\n"
                    f"Size: {file_size / 1024:.1f} KB\n\n"
                    f"This image has been processed and indexed.\n"
                    f"You can ask questions about the image content in your next message."
                )
                ...
                analysis = (
                    ...
                    f"Type: {extension.upper()}\n"
                    f"Size: {file_size / 1024:.1f} KB\n\n"
                    f"This file has been processed.\n"
                    f"You can reference this file in future messages."
                )
    ...
    def describe_image(self, image_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            response = ollama.chat(
                ...
                messages=[
                    {"role": "user", "content": description_prompt, "images": [image_data]}
                ]
            )
